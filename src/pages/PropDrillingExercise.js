...
const RootComponent = (props) => {
  ...
  const addProductToCart = (newProduct) => {
    const updatedProduct = cart.products.map((cartProduct) => {
      if (cartProduct.id === newProduct.id) {
        cartProduct.price = cartProduct.price + newProduct.price;
    ...
    setCart({
      products: updatedProduct, totalPrice: newTotalPrice
    })
  }
  ...
  const removeProductFromCart = (removeProduct) => {
    ...
    const updatedProduct = cart.products.map((cartProduct) => {
      if (cartProduct.id === removeProduct.id && cartProduct.qty > 0) {
        cartProduct.qty = cartProduct.qty - 1;
        cartProduct.price = cartProduct.price - removeProduct.price;
        newTotalPrice = newTotalPrice - removeProduct.price;
      }
      return cartProduct;
    });
    setCart({ products: updatedProduct, totalPrice: newTotalPrice });
  }
  // Step 3
      ...
      <Grid container spacing={2} p="1rem">
        <Grid item sm={6}>
          <ProductOne product={props.products[0]}
            addProduct={props.addProduct} removeProduct={props.removeProduct}
          />
        </Grid>
        <Grid item sm={6}>
